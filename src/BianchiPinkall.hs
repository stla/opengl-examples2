module BianchiPinkall
  (main)
  where
import           BianchiPinkall.Data
...
white,discord,fuchsia :: Color4 GLfloat
...
display context zoom alpha = do
  ...
  renderPrimitive Triangles $ do
    ...
    mapM_ drawTriangle lowerTriangles
  renderPrimitive Triangles $ do 
    materialDiffuse Front $= fuchsia
    mapM_ drawTriangle upperTriangles
  swapBuffers
  where
...
idle anim save delay snapshots alpha = do
  ...
  when a $ do
    ...
    when (s && ppmExists && snapshot < 360) $ do
      let ppm = printf "ppm/bianchipinkall%04d.ppm" snapshot
