...
mM1 = map (\(pt,pt') -> coneMesh pt pt' 0.06 0.06 3 30) edges1
mM2 = map (\(pt,pt') -> coneMesh pt pt' 0.06 0.06 3 30) edges2
mM3 = map (\(pt,pt') -> coneMesh pt pt' 0.06 0.06 3 30) edges3
mM4 = map (\(pt,pt') -> coneMesh pt pt' 0.06 0.06 3 30) edges4
mM5 = map (\(pt,pt') -> coneMesh pt pt' 0.06 0.06 3 30) edges5
...
idle anim save delay snapshots alpha = do
  ...
  when an $ do
    ...
    when (s && ppmExists && snapshot < 360) $ do
      let ppm = printf "ppm/fivetetrahedra%04d.ppm" snapshot
