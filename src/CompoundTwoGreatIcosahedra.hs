module CompoundTwoGreatIcosahedra
  where
import           CompoundTwoGreatIcosahedra.Data
...
white,black,gold,whitesmoke :: Color4 GLfloat
white      = Color4    1    1    1    1
black      = Color4    0    0    0    1
gold       = Color4    1 0.84    0    1
whitesmoke = Color4    1 0.96 0.96 0.96
...
display context = do
  ...
  forM_ rvertices $ \v -> preservingMatrix $ do
    ...
    renderObject Solid $ Sphere' 0.05 15 15
  drawEdges meshesAndMatrices gold
  drawEdges meshesAndMatrices' whitesmoke
  swapBuffers
  where
    drawEdges mM col = 
...
resize zoom s@(Size w h) = do
  ...
  lookAt (Vertex3 0 0 (5.2+zoom)) (Vertex3 0 0 0) (Vector3 0 1 0)
...
idle anim save delay snapshots alpha = do
  ...
  when an $ do
    ...
    when (s && ppmExists && snapshot < 360) $ do
      let ppm = printf "ppm/twogreaticosahedra%04d.ppm" snapshot
...
main = do
  ...
  _ <- createWindow "Compound of two great icosahedra"
  ...
  putStrLn "*** Compound of two great icosahedra ***\n\
        \    To quit, press q.\n\
