...
vertices = map flatten [(u,v) | u <- poly1, v <- poly2]
  ...
  flatten ((u1,u2),(v1,v2)) = (u1,v1,u2,v2)
...
edges_ij (i,j) = map fromJust $ filter isJust [e1, e2, e3, e4]
  where
    e = (i, j)
    c1 = (i, (j-1) `mod` n2)
    e1 = if e<c1 then Just c1 else Nothing
    c2 = (i, (j+1) `mod` n2)
    e2 = if e<c2 then Just c2 else Nothing
    c3 = ((i-1) `mod` n1, j)
    e3 = if e<c3 then Just c3 else Nothing
    c4 = ((i+1) `mod` n1, j)
    e4 = if e<c4 then Just c4 else Nothing
