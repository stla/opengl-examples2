...
type NPoint = (Vertex3 GLfloat, Normal3 GLfloat, Color4 GLfloat, Color4 GLfloat)
...
display context = do
  ...
  swapBuffers
  where
    drawTriangle ((v1,n1,col1,col1'),(v2,n2,col2,col2'),(v3,n3,col3,col3')) = do
      materialDiffuse Front $= col1
      materialDiffuse Back $= col1'
      normal n1
      vertex v1
      materialDiffuse Front $= col2
      materialDiffuse Back $= col2'
      normal n2
      vertex v2
      materialDiffuse Front $= col3
      materialDiffuse Back $= col3'
      normal n3
      vertex v3
...
idle anim delay save snapshots rot3 = do
    ...
    when a $ do
      ...
      when (s && ppmExists && snapshot < 360) $ do
        let ppm = printf "ppm/horsesaddle%04d.ppm" snapshot
...
main = do
  ...
  depthFunc $= Just Less
  shadeModel $= Smooth
  saddle <- newIORef $ allTriangles (nu,nv)
  rot1 <- newIORef 0.0
  rot2 <- newIORef 0.0
  rot3 <- newIORef 0.0
  zoom <- newIORef 0.0
  ...
  displayCallback $= display Context {
    ...
    contextRot3 = rot3,
    contextZoom = zoom,
    contextTriangles = saddle
  }   
  reshapeCallback $= Just (resize 0)
  keyboardCallback $= 
    Just (keyboard rot1 rot2 rot3 zoom anim delay save)
  idleCallback $= Just (idle anim delay save snapshots rot3)
  putStrLn "*** Horse saddle ***\n\
        \    To quit, press q.\n\
        \    Scene rotation:\n\
        \        e, r, t, y, u, i\n\
        \    Zoom: l, m\n\
