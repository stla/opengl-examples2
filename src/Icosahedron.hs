...
display context = do
  ...
  forM_ (zip facets (infinityOfColors 0 1)) $ \((i,j,k),col) -> 
    renderPrimitive Triangles $ do
      materialDiffuse Front $= col
      drawTriangle (vertices!!i, vertices!!j, vertices!!k)
  swapBuffers
  where
    drawTriangle (v1,v2,v3) = do
      normal $ triangleNegNormal (v1, v2, v3)
      vertex (tripletToVertex3 v1)
      vertex (tripletToVertex3 v2)
      vertex (tripletToVertex3 v3)
...
idle anim save delay snapshots alpha = do
  ...
  when an $ do
    ...
    when (s && ppmExists && snapshot < 360) $ do
      let ppm = printf "ppm/icosahedron%04d.ppm" snapshot
