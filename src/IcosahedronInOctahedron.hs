module IcosahedronInOctahedron
  where
...
import           IcosahedronInOctahedron.Data
...
meshesAndMatrices = map (\(pt,pt') -> coneMesh pt pt' 0.03 0.03 3 30) oedges'
...
display context = do
  ...
  forM_ overtices' $ \v -> preservingMatrix $ do
    translate v
    materialDiffuse Front $= black
    renderObject Solid $ Sphere' 0.05 15 15
  drawEdges meshesAndMatrices black
  forM_ (zip ifacets (infinityOfColors 20 1)) $ \((i,j,k),col) ->
    renderPrimitive Triangles $ do
    materialDiffuse Front $= col
    normal (triangleNegNormal (ivertices!!i,ivertices!!j,ivertices!!k))
    vertex (tripletToVertex3 $ ivertices!!i)
    vertex (tripletToVertex3 $ ivertices!!j)
    vertex (tripletToVertex3 $ ivertices!!k)
  swapBuffers
  where
    drawEdges mM col = 
...
idle anim save delay snapshots alpha = do
  ...
  when an $ do
    ...
    when (s && ppmExists && snapshot < 360) $ do
      let ppm = printf "ppm/icosahedroninoctahedron%04d.ppm" snapshot
...
main = do
  ...
  _ <- createWindow "Icosahedron within octahedron"
  windowSize $= Size 500 500
  initialDisplayMode $= [RGBAMode, DoubleBuffered, WithDepthBuffer]
  clearColor $= whitesmoke
  ...
  putStrLn "*** Icosahedron within octahedron ***\n\
        \    To quit, press q.\n\
