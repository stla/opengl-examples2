...
type NPoint = (Vertex3 GLfloat, Normal3 GLfloat)
type NTriangle = (NPoint, NPoint, NPoint)
type PairedTriangles = (NTriangle,NTriangle)

cyclide :: Float -> Float -> [PairedTriangles]
...
keyboard rot1 rot2 rot3 zoom phi anim save delay triangles char _ = do
  case char of
    ...
    'h' -> do
      phi $~! (+0.2)
      phi' <- get phi
      writeIORef triangles (cyclide 0 phi', cyclide (2*pi/3) phi', cyclide (4*pi/3) phi')
    'n' -> do
...
main = do
  ...
  alpha <- newIORef 0.0
  let phi = 1.2
      triangles = (cyclide 0 phi, cyclide (2*pi/3) phi, cyclide (4*pi/3) phi)
